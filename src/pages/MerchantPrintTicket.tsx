...
const MerchantPrintTicket = () => {
  ...
  const { data: ticketData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Fetch ticket header
      const tRes = await fetch(`${API_URL}/tickets`, { headers: getAuthHeader() });
      const tickets = (await tRes.json()).data;
      const ticket = tickets.find((t: any) => t.id.toString() === ticketId);

      // Fetch ticket details
      const dRes = await fetch(`${API_URL}/ticket-details?ticketId=${ticketId}`, { headers: getAuthHeader() });
      const details = (await dRes.json()).data;

      // Fetch service & categories for metadata
      const sRes = await fetch(`${API_URL}/merchant-services`, { headers: getAuthHeader() });
      const service = (await sRes.json()).data.find((s: any) => s.id === ticket.merchant_service_id);
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="flex items-center justify-between print:hidden">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-3xl font-bold text-slate-900">Print Tickets</h1>
            </div>
            <Button onClick={handlePrint} className="bg-indigo-600 hover:bg-indigo-700 gap-2 h-12 px-6 rounded-xl shadow-lg">
