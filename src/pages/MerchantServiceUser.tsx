...
const MerchantServiceUser = () => {
  ...
  // Fetch service user mappings for this merchant (server-side paginated)
  const {
    data: mappingsResponse,
    isLoading: isLoadingMappings,
    isFetching: isFetchingMappings,
  } = useQuery({
    queryKey: ["merchant-service-users", merchantId, page, pageSize],
    queryFn: async () => {
      if (!merchantId) return { data: [], pagination: { page: 1, pageSize, total: 0, totalPages: 1 } };
      ...
      const res = await fetch(
        `${API_URL}/merchant-service-users?${params.toString()}`,
        {
          headers: getAuthHeader(),
        },
      );
  ...
  const pagination = mappingsResponse?.pagination ?? {
    page: 1,
    pageSize,
    total: 0,
    totalPages: 1,
